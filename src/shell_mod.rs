...
impl ShellCommandLimitedDoubleQuotesSanitizerTrait for ShellCommandLimitedDoubleQuotesSanitizer {
    ...
    fn new(template: &str) -> ResultWithLibError<Self> {
        ...
        Ok(ShellCommandLimitedDoubleQuotesSanitizer {
            ...
            string_to_echo: String::new(),
            string_to_execute: String::new(),
        })
    }
    /// Replace placeholders with the value
    ///
    ...
    fn arg(&mut self, placeholder: &str, value: &str) -> ResultWithLibError<&mut Self> {
        ...
        if self.string_to_echo.is_empty() {
            self.string_to_echo = self.template.replace(placeholder, value);
        } else {
            self.string_to_echo = self.string_to_echo.replace(placeholder, value);
        }
        if self.string_to_execute.is_empty() {
            self.string_to_execute = self.template.replace(placeholder, value);
        } else {
    ...
    fn arg_secret(&mut self, placeholder: &str, value: &secrecy::SecretString) -> ResultWithLibError<&mut Self> {
        ...
        if self.string_to_echo.is_empty() {
            self.string_to_echo = self.template.replace(placeholder, "[REDACTED]");
        } else {
            self.string_to_echo = self.string_to_echo.replace(placeholder, "[REDACTED]");
        }
        if self.string_to_execute.is_empty() {
            self.string_to_execute = self.template.replace(placeholder, value.expose_secret());
        } else {
