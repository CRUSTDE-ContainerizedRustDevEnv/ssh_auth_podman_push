...
//!
//! Instead of that, copy and paste this module `docker_io_api_token_mod.rs` file into your project.  
...
//!
//! Beware that the versions of docker in `Cargo.toml` are not precisely pinpointed. In rust the symbol '=' means "the same major number equal or newer to". This means from one compilation to another, it can automatically change to a newer version without the programmer even noticing it.
//!
...
use cargo_auto_lib::{ShellCommandLimitedDoubleQuotesSanitizer, ShellCommandLimitedDoubleQuotesSanitizerTrait};
...
pub struct DockerIoConfig {
    pub docker_io_private_key_file_name: String,
}

/// Application state is initialized in the main() function.
///
/// And then is accessible all over the code.
pub static DOCKER_IO_CONFIG: std::sync::OnceLock<DockerIoConfig> = std::sync::OnceLock::new();

/// Application state (static) is initialized only once in the main() function.
///
/// And then is accessible all over the code.
pub fn docker_io_config_initialize() {
    if DOCKER_IO_CONFIG.get().is_some() {
        return;
    }

    let docker_io_config_json = std::fs::read_to_string("docker_io_config.json").unwrap();
    let docker_io_config: DockerIoConfig = serde_json::from_str(&docker_io_config_json).unwrap();
    let _ = DOCKER_IO_CONFIG.set(docker_io_config);
}
...
pub(crate) fn get_docker_io_secret_token(private_key_file_name: &str) -> anyhow::Result<SecretString> {
    // check if the plain-text file from `podman login` exists and warn the user
    // because it is a security vulnerability.
    // in WSL the var ${XDG_RUNTIME_DIR} = /run/user/1000/
    let file_credentials = "${XDG_RUNTIME_DIR}/containers/auth.json";
    if let Some(xdg_runtime_dir) = std::env::var_os("XDG_RUNTIME_DIR") {
        let xdg_runtime_dir = xdg_runtime_dir.to_string_lossy().to_string();
        let file_credentials_expanded = file_credentials.replace("${XDG_RUNTIME_DIR}", &xdg_runtime_dir);
        let file_credentials_expanded = camino::Utf8Path::new(&file_credentials_expanded);
        if file_credentials_expanded.exists() {
            eprintln!("{RED}Security vulnerability: Found the docker hub auth.json file with plain-text secret_token: {RESET}");
            eprintln!("{RED}{file_credentials_expanded}. It would be better to inspect and remove it. {RESET}");
    ...
    if !private_key_path_struct.exists() {
        ...
        println!(r#"{GREEN}ssh-keygen -t ed25519 -f {private_key_path_struct} -C "docker.io secret_token"{RESET}"#);
    ...
    if !encrypted_path_struct.exists() {
        ...
        println!("  {YELLOW}Never use 'podman login' to store this secret locally. It will store it in plain-text in the file $XDG_RUNTIME_DIR/containers/auth.json. {RESET}");
        println!("  {YELLOW}Plain-text for secrets in a well-known file is a big no-no. Every malware will just upload it in milliseconds. {RESET}");
...
#[allow(dead_code)]
pub fn push_to_docker_hub(image_url: &str) -> anyhow::Result<()> {
    ...
    let _registry = splitted.next().unwrap();
    let user_name = splitted.next().unwrap();

    #[derive(serde::Deserialize, serde::Serialize)]
    struct DockerIoApiConfig {
        docker_io_secret_token_key: String,
    }

    let secret_access_token = get_docker_io_secret_token(&DOCKER_IO_CONFIG.get().unwrap().docker_io_private_key_file_name)?;
    // the secret_access_token can be used in place of the password in --cred
    ShellCommandLimitedDoubleQuotesSanitizer::new(r#"podman push --creds "{user_name}:{secret_access_token}" "{image_url}" "#)
        .unwrap_or_else(|e| panic!("{e}"))
        .arg("{user_name}", user_name)
        .unwrap_or_else(|e| panic!("{e}"))
        .arg_secret("{secret_access_token}", &secret_access_token)
        .unwrap_or_else(|e| panic!("{e}"))
